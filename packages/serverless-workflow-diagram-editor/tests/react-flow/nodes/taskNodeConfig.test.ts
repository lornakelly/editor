...
describe("taskNodeConfig", () => {
  ...
  it("should have config for all terminal nodes", () => {
  ...
  it.each(terminalNodeTypes)(
    ...
    (terminal) => {
      ...
      expect(config.icon).toBeDefined();
      // labelKey must resolve to a real translation key in the dictionary
  ...
  describe("getNodeVisualConfig", () => {
    ...
    it.each(terminalNodeTypes)(
      "returns undefined for terminal node type %s (terminals are not in the visual config maps)",
      (terminal) => {
