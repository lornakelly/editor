...
export { CATCH_CONTAINER_NODE_TYPE } from "./taskNodeConfig";
...
/* Entry/exit nodes */
function TerminalNodeContent({ id, selected, type }: Omit<NodeContentProps, "badge">) {
  const { t } = useI18n();
  const config = terminalNodeConfigMap[type as TerminalNodeType];
  const Icon = config.icon;
  const label = t(config.labelKey);
  const isEntry = type === GraphNodeType.Entry;
  return (
    <div
      className={`dec-terminal-node ${selected ? "selected" : ""}`}
      aria-label={label}
      data-testid={`${type}-node-${id}`}
    >
      {isEntry ? (
...
export function EntryNode({ id, data, selected, type }: RF.NodeProps<EntryNodeType>) {
  return <TerminalNodeContent id={id} data={data} selected={selected} type={type} />;
}
